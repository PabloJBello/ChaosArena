...
namespace ControllerSystem.Platformer2D
{
    ...
    public class PlatformerAnimator : EntityAnimator
    {
        ...
        [SerializeField] private float _punchAnimLength = 0.3f;
        ...
        private float _lockInPunchUntilTime;
        ...
        private void HandleAnimation()
        {
            ...
            // If punching, override everything else until punch animation is done
            if (Time.time < _lockInPunchUntilTime)
            {
                state = Punch;
            }
            else
            {
                state = _fighterController.CurrentState == FighterController.States.Dead ? Dead : GetMovementState();
            }
        ...
        public void OnPunch()
        {
            motor.Rb.linearVelocity = new Vector2(0, motor.Rb.linearVelocity.y);

            SwitchAnimState(Punch);
            _lockInPunchUntilTime = Time.time + _punchAnimLength;
        }
    }
}